...
class ControllerStartupSeoUrl extends Controller {
	public function index() {
		...
		if (isset($this->request->get['_route_'])) {
			...
			foreach ($parts as $part) {
				...
				if ($query->num_rows) {
					...
					///////////////////////// -catstat
					if ($url[0] == 'catstat_id') {
						$this->request->get['catstat_id'] = $url[1];
					}
					if ($url[0] == 'categorystat_id') {
						$this->request->get['categorystat_id'] = $url[1];
					}
					if ($url[0] == 'categorystat_id') {
						if (!isset($this->request->get['pathc'])) {
							$this->request->get['pathc'] = $url[1];
							} else {
							$this->request->get['pathc'] .= '_' . $url[1];
						}
					}
					////////////////////////////////
					...
					if ($url[0] == 'information_id') {
						$this->request->get['information_id'] = $url[1];
                                                                             ...
                                                                             ////////////////////////////////////////////////////////////
					if ($query->row['query'] && $url[0] != 'information_id' && $url[0] != 'catstat_id' && $url[0] != 'categorystat_id' && $url[0] != 'manufacturer_id' && $url[0] != 'category_id' && $url[0] != 'product_id') {
			...
			if (!isset($this->request->get['route'])) {
				if (isset($this->request->get['product_id'])) {
				  ...
				  elseif (isset($this->request->get['catstat_id'])) {
					$this->request->get['route'] = 'extension/module/catstat';
				}////////
				elseif (isset($this->request->get['categorystat_id'])) {
					$this->request->get['route'] = 'extension/module/catstat/category';
				}
		...
		// Redirect 301   
			...
			if ($this->request->get['route'] == 'product/product' && isset($this->request->get['product_id'])) {
				...
				(isset($this->request->get['page'])) $arg = $arg . '?page=' . (int)$this->request->get['page'];
			} 
			...
			///////////////////////////
			elseif ($this->request->get['route'] == 'extension/module/catstat' && isset($this->request->get['pathc'])) {
				$categorys1_id = explode('_', $this->request->get['pathc']);
				
				$cat_path = '';
				foreach ($categorys1_id as $category1_id) {
					$query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "seo_url` WHERE `query` = 'categorystat_id=" . (int)$category1_id . "' AND `store_id` = '" . (int)$this->config->get('config_store_id') . "' AND `language_id` = '" . (int)$this->config->get('config_language_id') . "'");   
					if ($query->num_rows && $query->row['keyword'] /**/ ) {
						$cat_path .= '/' . $query->row['keyword'];
					} else {
						$cat_path = false;
	...
	public function rewrite($link) {
		...
		foreach ($data as $key => $value) {
			if (isset($data['route'])) {
				if (($data['route'] == 'product/product' && $key == 'product_id') || (($data['route'] == 'product/manufacturer/info' || $data['route'] == 'product/product') && $key == 'manufacturer_id') || ($data['route'] == 'information/information' && $key == 'information_id')|| ($data['route'] == 'extension/module/catstat' && $key == 'catstat_id') || ($data['route'] == 'extension/module/catstat/category' && $key == 'categorystat_id')) {
				...
				elseif ($key == 'pathc') {
					$categories1 = explode('_', $value);

					foreach ($categories1 as $category1) {
						$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "seo_url WHERE `query` = 'categorystat_id=" . (int)$category1 . "' AND store_id = '" . (int)$this->config->get('config_store_id') . "' AND language_id = '" . (int)$this->config->get('config_language_id') . "'");
